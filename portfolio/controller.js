...
export async function  getPortfolioHistory(portfolio,startDate,endDate){
    let historical={};
    ...
    try {
      for( let d=new Date(startDate);d<endDate;d.setDate(d.getDate()+1)){
        ...
        for(let asset of Object.entries(pfAssets)){
          dailyPfValue += await getAssetDailyValue(historical,asset,assetsPrevDayVal,d,dString,eString);
        }
        let toAdd=true;
...
export async function  getPortfoliosHistory(portfolios,startDate,endDate){
        let historical={};
        let portfolioHistoricalValue=[];
        let assetsPrevDayVal={};
        let initSkip=true;
        ...
        try {
          for( let d=new Date(startDate);d<endDate;d.setDate(d.getDate()+1)){
            ...
            for(let pf of portfolios){
                ...
                for(let asset of Object.entries(pfAssets)){
                    !overallAssets[asset[0]]?overallAssets[asset[0]]=asset[1]:overallAssets[asset[0]]+=asset[1];
                }
            }
...
export async function getAssetInPortfolioValueHistory(portfolio,assetSymbol,startDate,endDate){
    ...
    try {
      for( let d=new Date(startDate);d<endDate;d.setDate(d.getDate()+1)){
        ...
        let dString=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`
        assetsPrevDayVal['date']=d.toDateString();
        if(!pfAssets[assetSymbol]){return null;}
        let asset=[assetSymbol,pfAssets[assetSymbol]];
        let dailyValue=await getAssetDailyValue(historical,asset,assetsPrevDayVal,d,dString,eString);
        let toAdd=true;
        let assetsPrevList=Object.entries(assetsPrevDayVal);
        assetsPrevList.shift();
        if (initSkip){
...
async function getAssetDailyValue(historical,asset,assetsPrevDayVal,d,dString,eString){
  let dailyPfValue=0;
